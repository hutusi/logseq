...
- ## 感知机：
	...
	- 训练数据和测试数据：
	- 使用训练数据进行学习，使用测试数据评价训练得到的模型的实际能力，训练数据也可以称为监督数
	- ### 损失函数：
...
- ## 卷积神经网络
	...
	- ### 卷积层
		- 全连接层存在的问题：输入数据是有形状的数据，比如三维数据，但向全连接层输入时，三维数据被拉平为一维数据。而卷积层可以保持数据形状不变。
		- 卷积运算：卷积层进行的处理的就是卷积运算。卷积运算相当于图像处理中的“滤波器运算”。
	- ### 池化层
		- 池化是缩小高、长方向上的空间的运算。
	- ### 卷积层和池化层的实现
		- 略
